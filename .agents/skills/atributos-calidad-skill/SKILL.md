...
description: >-
  Skill de arquitectura de software basada en los métodos del SEI (Software Engineering Institute)
  y en Bass, Clements & Kazman, *Software Architecture in Practice*, 4ta Ed. Usála para:
  1) Generar y refinar escenarios de atributos de calidad con el template de 6 partes del SEI
  (fuente del estímulo, estímulo, artefacto, ambiente, respuesta, medida de la respuesta).
  2) Chequear si un escenario de calidad dado está completo, detectar qué partes faltan o son
  ambiguas y proponer cómo completarlo (técnica de straw man response measure).
  3) Elaborar y priorizar un árbol de utilidad (utility tree) con la tupla
  (Importancia para el negocio, Dificultad técnica).
  Cubre disponibilidad, performance, modificabilidad, seguridad, usabilidad, interoperabilidad
  y escalabilidad, y se apoya en los métodos ADD, ATAM y QAW.
---
...
- **Atributo de Calidad:** [Disponibilidad | Modificabilidad | Performance | Seguridad | Usabilidad | Testeabilidad | Interoperabilidad | Escalabilidad]
...
1. **Nodo Raíz:** `Utilidad` (utilidad global del sistema).
