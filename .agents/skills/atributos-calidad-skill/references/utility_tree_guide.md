...
2. **Nivel 1: Atributos de Calidad (Quality Attributes)**:
   - Los grandes factores de calidad arquitectónica (ej. *Performance*, *Modificabilidad*, *Seguridad*, *Disponibilidad*, *Usabilidad*, *Interoperabilidad*).
3. **Nivel 2: Categorías / Sub-atributos (Refinement / Quality Attribute Sub-factors)**:
   - Aspectos específicos de cada atributo (ej. dentro de *Performance*: *Latencia*, *Throughput*, *Carga Pico*; dentro de *Seguridad*: *Autenticación*, *Auditoría*, *Confidencialidad*).
...
```text
                                     ┌── Latencia ────────────── (H, H) ─── Escenario 1: El tiempo de respuesta de búsqueda...
                ┌── Performance ─────┤
                │                    └── Throughput ──────────── (M, L) ─── Escenario 2: El sistema debe procesar 500 tx/s...
                │
                │                    ┌── Autenticación ───────── (H, M) ─── Escenario 3: Tras 3 intentos fallidos de login...
Utilidad (Raíz) ┼── Seguridad ───────┤
                │                    └── Auditoría ───────────── (H, L) ─── Escenario 4: Toda modificación de tarifas queda...
                │
                │                    ┌── Failover ────────────── (H, H) ─── Escenario 5: Si el servidor principal cae...
                └── Disponibilidad ──┤
                                     └── Pérdida de conexión ─── (M, M) ─── Escenario 6: Si se pierde la conexión GPS...
```
...
```mermaid
graph LR
    U[Utilidad] --> PERF[Performance]
    U --> SEG[Seguridad]
    U --> DISP[Disponibilidad]

    PERF --> PERF_A[Latencia]
    PERF_A --> ESC1["(H, M) Búsqueda en mapa < 2s"]

    SEG --> SEG_A[Autenticación]
    SEG_A --> ESC2["(H, H) Verificación QR y token JWT en < 500ms"]

    DISP --> DISP_A[Failover]
    DISP_A --> ESC3["(H, H) Backup ante caída de nodo en < 30s"]
```
